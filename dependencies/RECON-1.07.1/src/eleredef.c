...
int find_prim(IMG_NODE_t *nd, float cutoff, int32_t end1, int32_t end2, int32_t efl1, int32_t efl2, int32_t al1, int32_t al2, int32_t score, int *pmarkp, int32_t *sc, short *d) {
  ...
  if (nd->children) {
    ...
    if ( (1.0*al1/efl1 > cutoff || 1.0*al2/efl2 > cutoff) && (efl1-al1 < 30 || efl2-al2 < 30) ) {
      ...
      if ( (al1+al2) == 0 )
      {
         // RMH: A divide by zero error has occured a few times at this point.  I looked extensively
         //      through the code and cannot determine what is causing this -- although I have a 
         //      reliable test case to explore this further.  For now this is the only workaround.
